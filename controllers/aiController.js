...
exports.chat = asyncHandler(async (req, res) => {
  const data = await aiService.chat({ userId: req.user._id, conversationId: req.body.conversationId, message: req.body.message });
  res.json({ success: true, data });
});

exports.listConversations = asyncHandler(async (req, res) => {
  const data = await aiService.listConversations(req.user._id);
  res.json({ success: true, data });
});

exports.getConversation = asyncHandler(async (req, res) => {
  const data = await aiService.getConversation(req.user._id, req.params.id);
  res.json({ success: true, data });
});
...
exports.listRecaps = asyncHandler(async (req, res) => {
  const data = await aiInsights.listRecapsForAstrologer(req.user._id, {
...
exports.getRecap = asyncHandler(async (req, res) => {
  const data = await aiInsights.getRecapForAstrologer(req.user._id, req.params.id);
  res.json({ success: true, data });
});

// Astrologer: edit before approving.
exports.editRecap = asyncHandler(async (req, res) => {
  const data = await aiInsights.editRecap(req.user._id, req.params.id, {
...
exports.approveRecap = asyncHandler(async (req, res) => {
  const data = await aiInsights.approveRecap(req.user._id, req.params.id, { keepSuggestionIds: req.body.keepSuggestionIds });
  res.json({ success: true, data });
});

exports.rejectRecap = asyncHandler(async (req, res) => {
  const data = await aiInsights.rejectRecap(req.user._id, req.params.id);
  res.json({ success: true, data });
});

// User: the published recap for one of their sessions (null if none).
exports.userRecap = asyncHandler(async (req, res) => {
  const data = await aiInsights.getRecapForUser(req.user._id, req.params.sessionId);
  res.json({ success: true, data });
});
...
exports.optimizeProfile = asyncHandler(async (req, res) => {
  const data = await aiInsights.optimizeProfile(req.user._id);
  const usage = await aiInsights.optimizerUsage(req.user._id);
